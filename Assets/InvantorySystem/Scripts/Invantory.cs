...
public class Invantory : MonoBehaviour
{
	...
	private void MoveSlot (bool goUp)
	{
	...
	private void InitGUI ()
	{
		...
		if (even) {
			...
			oldPos.x -= (tempGameObject.GetComponent<RectTransform> ().rect.width / 2 + padding / 2);
		...
		for (int i = 0; i <= maxInvantorySlots / 2; i++) {
			...
			var oldPos = tempGameObject.GetComponent<RectTransform> ().position;
			oldPos.x += i * tempGameObject.GetComponent<RectTransform> ().rect.width + (i * padding) + (even ? (tempGameObject.GetComponent<RectTransform> ().rect.width / 2) + padding / 2 : 0);
			tempGameObject.GetComponent<RectTransform> ().position = oldPos;
